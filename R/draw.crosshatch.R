#crosshatch function from https://imaddowzimet.github.io/crosshatch/
# Define function
draw.crosshatch <- function(data, width, pattern=c("vertical", "horizontal", "crosshatch")) {
  # data <- fifty_states_estimated[[2]][[1]]
  # data <- t2[[2]]
  # setDT(data)
  # data <- data[,.(lat,long,order)]
  # pattern='crosshatch'
  # width=0.5
  ...
  if ((data$long[leftmostpoint == 1]+width) >= (data$long[rightmostpoint == 1]- width)) {
    ...
    find.top.lat <- function(index_longitude) {
      # index_longitude <- index_long[1] # test
      ...
      if(length(long_lat)>1 & round(sum(long_lat)/length(long_lat),3)==round(long_lat[1],3)){
        long_lat <- long_lat[1]
      }
      long_lat
    }
    ...
    find.bottom.lat <- function(index_longitude) {
      # index_longitude <- index_long[15] # test
  ...
  if ((data$lat[lowestpoint == 1]+width) >= (data$lat[highestpoint == 1] - width)) {
    ...
    find.left.long <- function(index_latitude) {
      ...
      if(length(lat_long)>1 & round(sum(lat_long)/length(lat_long),3)==round(lat_long[1],3)){
        lat_long <- lat_long[1]
      }
      lat_long
    }
